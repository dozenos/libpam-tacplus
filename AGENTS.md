...
PAM module providing TACACS+ authentication, authorization, and accounting for DozenOS logins, plus the `libtac` TACACS+ protocol library and the `tacc` CLI client. Pairs with `libnss-tacplus` (NSS lookups) and `libtacplus-map` (user mapping) to deliver TACACS+ login on DozenOS.
...
Authentication building block of the DozenOS image. Provides the `libtac` library that `libnss-tacplus` links against, and the PAM module that `libtacplus-map` complements for `/etc/passwd`-less TACACS+ logins. Built via the internal build-packages workflow; consumed by `dozenos/dozenos-build`.
...
- README is upstream's; DozenOS-specific behaviour is encoded only by the build flags and patches applied here.
